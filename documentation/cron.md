...
The endpoint is read-only and the job is idempotent. Missing monitor/release configuration, timeout, or a non-2xx required dependency response fails the run. Cloud quantum is reported as not configured when the optional backend is disabled; when configured, its health becomes required. GitHub Actions history is retained as the operational record.
