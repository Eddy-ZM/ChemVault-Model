...
export function structureToXyz(structureData: string | null | undefined, format: StructureFormat | null | undefined) {
  if (!structureData?.trim()) return null;

  const parsed = parseStructure(structureData, String(format || '').toLowerCase());
  if (!parsed || parsed.atoms.length === 0) return null;

  return [
    String(parsed.atoms.length),
    'Generated by ChemVault Model',
    ...parsed.atoms.map((atom) => `${atom.element} ${formatCoordinate(atom.x)} ${formatCoordinate(atom.y)} ${formatCoordinate(atom.z)}`)
  ].join('\n');
}
